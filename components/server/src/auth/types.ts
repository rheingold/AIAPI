...
export interface AuthCredentials {
  ...
  apiKey?: string;
  // certificate
  clientCert?: string; // PEM X.509 client cert from TLS handshake
  // oauth callback
  oauthCode?: string;
  oauthState?: string;
...
// ─── Settings sub-types (for auth section of dashboard-settings.json) ─────────
...
export interface SamlConfig {
  entryPoint: string;
  issuer: string;
  cert: string;           // PEM or "file:/path"
  privateKey?: string;    // PEM or "file:/path"
  callbackUrl: string;
  usernamePath: string;
  groupsPath?: string;
...
export interface AuthConfig {
  ...
  apikey?: { defaultUser: string };
  certificate?: { caPath: string; requireClientCert: boolean };
  oauth?: OAuthConfig;
  saml?: SamlConfig;
