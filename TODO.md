...
- [x] **ADR-017-P3 (MCP hierarchy Phase 3 — restore helper tools to root):** Restored `...this.helperRegistry.toMcpTools()` to `handleToolsList()` in `mcpServer.ts`. Direct helper tools (BrowserWin, KeyWin, MSOfficeWin, LibreOfficeWin) are now visible in MCP `tools/list` for direct access. NativeWin actions (EXEC_CMD, FS_READ, FS_WRITE, FS_LIST, FETCH_WEBPAGE, FETCH_WEBPAGE_RENDER) remain internal-only, accessible exclusively via `AutomateUI { helper:"NativeWin", action:"..." }`. Updated `mcpServer.integration.test.ts` to verify helpers are present in root list. **Deployed:** commit 9fd2480, deployed to production service (port 4457) via `update-service.ps1`. Verified: 8 tools in tools/list, KeyWin.LISTWINDOWS returns 18 windows. *(done 2026-07-06)*

- [x] **FETCH-ESCALATE (Auto-escalation for login/captcha detection):** Enhanced `FETCH_WEBPAGE` handler in `mcpServer.ts` to automatically escalate to `FETCH_WEBPAGE_RENDER` (interactive browser mode) when a login form is detected in the HTTP response. Uses existing `browserObstacleProfiles.ts` profiles for captcha/login/consent detection. Interactive-by-default: visible browser window (`headless:false`, `background:false`) routes to active user session via WinSvcBridge in Session 0. Result includes `_escalated`, `_escalationReason`, and `obstacle` metadata. Caller can suppress with `autoEscalate:false` option. Documented in CONVENTIONS.md §5.8. **Bug fix:** Added `?? {}` fallback for undefined `fetchOptions` to prevent runtime error. **Tested:** example.com (no escalation), github.com/login (escalates correctly with high-confidence login form detection). **Deployed:** Production verification: google.com (81KB content, no escalation), github.com/login (escalated=true, "Login form detected", confidence=high). *(done 2026-07-06)*

- [x] **ADR-017-P2 (MCP hierarchy Phase 2 — remove legacy tools from tools/list):** Removed 8 legacy stubs (`queryTree`, `clickElement`, `setProperty`, `readProperty`, `getProviders`, `listWindows`, `launchProcess`, `terminateProcess`) from `handleToolsList()`. Also removed 5 NativeWin flat tools (`exec_cmd`, `fs_read`, `fs_write`, `fs_list`, `fetch_webpage`) from root — now exclusively accessible via `AutomateUI { helper:"NativeWin", action:"..." }`. `handleToolsCall()` dispatch kept for all 13 removed tools with migration-hint `-32601` errors. `AutomateUI` NativeWin branch wired to `execCmd`/`fsRead`/`fsWrite`/`fsList`/`fetchWebpage`. D9 `hs9-tool-hierarchy` updated: count range 4..15, asserts NativeWin flat tools absent from root, MCP content-envelope unwrapping fixed in hs5/hs6/hs9. **11/11 D9 tests pass.** *(done 2026-06-05)*
