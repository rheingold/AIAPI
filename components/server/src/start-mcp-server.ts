...
async function main() {
  ...
  // Load private key for helper HMAC authentication (optional — skipped gracefully
  // when security is not yet initialised or user presses Enter with no password).
  const pkBytes = await loadCryptoCredentials();

  console.log('\n=== Starting MCP Server ===');
  // Create server without engine - it will create its own with session token.
  // Pass pkBytes so helper daemons can derive per-session HMAC keys via HKDF.
  const server = new MCPServer(undefined, port, pkBytes ?? undefined);
  ...
  await dashboard.initAuth();   // loads auth config from dashboard-settings.json; no-op if auth.mode is missing
