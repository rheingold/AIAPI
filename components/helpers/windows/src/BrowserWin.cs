...
namespace BrowserWin
{
    class Program
    {
        ...
        static int CmdFocusBrowser(string browser)
        {
            IntPtr hwnd = IntPtr.Zero;
            foreach (string pname in GetBrowserProcessNames(browser))
            {
        ...
        static string DetermineCommandType(string cmd)
        {
            ...
            if (cmd.Equals("{FOCUS}",         StringComparison.OrdinalIgnoreCase)) return "FOCUS";
        ...
        static string GetApiSchema()
        {
            ...
            sb.AppendLine("    { \"name\": \"LISTBROWSERS\", \"description\": \"List all running Chromium-family browser processes (Chrome, Edge, Brave, Opera)\", \"parameters\": [], \"examples\": [\"action=LISTBROWSERS\"] },");
            sb.AppendLine("    { \"name\": \"LAUNCH\", \"description\": \"Start a browser with --remote-debugging-port so CDP commands work, or reuse an existing debug window."
                ...
                + "   \"enum\": [\"visible\", \"headless\"] } ], \"examples\": [\"action=LAUNCH\", \"action=LAUNCH path=visible\", \"action=LAUNCH path=headless\"] },");
            sb.AppendLine("    { \"name\": \"NAVIGATE\", \"description\": \"Navigate the active page to a URL\", \"parameters\": [ { \"name\": \"url\", \"type\": \"string\", \"required\": true } ], \"examples\": [\"action=NAVIGATE path=https://example.com\"] },");
            sb.AppendLine("    { \"name\": \"READ\", \"description\": \"Read the current page title, URL and meta-description\", \"parameters\": [], \"examples\": [\"action=READ\"] },");
            sb.AppendLine("    { \"name\": \"QUERYTREE\", \"description\": \"Query the DOM tree up to the specified depth (default 3)\", \"parameters\": [ { \"name\": \"depth\", \"type\": \"integer\", \"required\": false, \"default\": 3 } ], \"examples\": [\"action=QUERYTREE\", \"action=QUERYTREE path=5\"] },");
            sb.AppendLine("    { \"name\": \"CLICKID\", \"description\": \"Click a DOM element matching a CSS selector\", \"parameters\": [ { \"name\": \"selector\", \"type\": \"string\", \"required\": true } ], \"examples\": [\"action=CLICKID path=#submitBtn\", \"action=CLICKID path=.login-button\"] },");
            sb.AppendLine("    { \"name\": \"CLICKNAME\", \"description\": \"Click a button/link whose visible text matches the given string\", \"parameters\": [ { \"name\": \"text\", \"type\": \"string\", \"required\": true } ], \"examples\": [\"action=CLICKNAME path=Log in\", \"action=CLICKNAME path=Accept all\"] },");
            sb.AppendLine("    { \"name\": \"FILL\", \"description\": \"Set the value of an input field.\", \"parameters\": [ { \"name\": \"selector\", \"type\": \"string\", \"required\": true }, { \"name\": \"value\", \"type\": \"string\", \"required\": true } ], \"examples\": [\"action=FILL path=#email value=user@example.com\", \"action=FILL path=input[name=q] value=search term\"] },");
            sb.AppendLine("    { \"name\": \"EXEC\", \"description\": \"Evaluate a JavaScript expression and return its string result\", \"parameters\": [ { \"name\": \"script\", \"type\": \"string\", \"required\": true } ], \"examples\": [\"action=EXEC path=document.title\", \"action=EXEC path=window.scrollY\"] },");
            sb.AppendLine("    { \"name\": \"READELEM\", \"description\": \"Read the visible text or value of a specific DOM element via CSS selector (CDP) or UIA fallback.\", \"parameters\": [ { \"name\": \"selector\", \"type\": \"string\", \"required\": true } ], \"examples\": [\"action=READELEM path=#result\", \"action=READELEM path=input[name=total]\"] },");
            sb.AppendLine("    { \"name\": \"SENDKEYS\", \"description\": \"Send keyboard input to the browser window (UIA). Supports special keys: {ENTER}, {TAB}, {ESC}, {CTRL+C}, etc.\", \"parameters\": [ { \"name\": \"keys\", \"type\": \"string\", \"required\": true } ], \"examples\": [\"action=SENDKEYS value=hello world\", \"action=SENDKEYS value={CTRL+A}{CTRL+C}\"] },");
            sb.AppendLine("    { \"name\": \"PAGESOURCE\", \"description\": \"Retrieve the raw HTML source of the current page via Ctrl+U/Ctrl+A/Ctrl+C clipboard trick (UIA; works on every browser, no debug port required).\", \"parameters\": [], \"examples\": [\"action=PAGESOURCE\"] },");
            sb.AppendLine("    { \"name\": \"SCREENSHOT\", \"description\": \"Capture a PNG screenshot of the current browser page via CDP Page.captureScreenshot. Optional parameter: file path to save (default: %TEMP%\\\\aiapi_screenshot_<timestamp>.png). Returns the saved file path and byte size.\", \"parameters\": [ { \"name\": \"path\", \"type\": \"string\", \"required\": false } ], \"examples\": [\"action=SCREENSHOT\", \"action=SCREENSHOT path=C:\\\\Users\\\\me\\\\shot.png\"] },");
            sb.AppendLine("    { \"name\": \"NEWPAGE\", \"description\": \"Open a new browser tab; optionally navigate it to a URL\", \"parameters\": [ { \"name\": \"url\", \"type\": \"string\", \"required\": false } ], \"examples\": [\"action=NEWPAGE\", \"action=NEWPAGE path=https://example.com\"] },");
            sb.AppendLine("    { \"name\": \"COOKIES\", \"description\": \"Manage browser cookies. Actions: get | clear | set:<name=value;domain=...>\", \"parameters\": [ { \"name\": \"action\", \"type\": \"string\", \"required\": true } ], \"examples\": [\"action=COOKIES path=get\", \"action=COOKIES path=clear\", \"action=COOKIES path=set value=SOCS=accept\"] },");
            sb.AppendLine("    { \"name\": \"KILL\", \"description\": \"Terminate the target browser process\", \"parameters\": [], \"examples\": [\"action=KILL\"]} ,");
            sb.AppendLine("    { \"name\": \"KEYDOWN\", \"description\": \"Hold a modifier key down via CDP Input.dispatchKeyEvent (or SendInput UIA fallback). Use with KEYUP to build chords. Param: Ctrl | Alt | Shift | Win.\", \"parameters\": [ { \"name\": \"key\", \"type\": \"string\", \"required\": true } ], \"examples\": [\"action=KEYDOWN path=Ctrl\", \"action=KEYDOWN path=Alt\"] },");
            sb.AppendLine("    { \"name\": \"KEYUP\", \"description\": \"Release a held modifier key. Always pair with a prior KEYDOWN for the same key.\", \"parameters\": [ { \"name\": \"key\", \"type\": \"string\", \"required\": true } ], \"examples\": [\"action=KEYUP path=Ctrl\", \"action=KEYUP path=Alt\"] },");
            sb.AppendLine("    { \"name\": \"KEYPRESS\", \"description\": \"Atomic keydown+keyup for function or navigation keys. Supported: F1-F12, HOME, END, PAGEUP, PAGEDOWN, INSERT, DELETE, ENTER, TAB, ESC, BACK, LEFT, RIGHT, UP, DOWN, APPS.\", \"parameters\": [ { \"name\": \"key\", \"type\": \"string\", \"required\": true } ], \"examples\": [\"action=KEYPRESS path=F5\", \"action=KEYPRESS path=HOME\"] },");
            sb.AppendLine("    { \"name\": \"RIGHTCLICK\", \"description\": \"Right-click at screen coordinates via CDP Input.dispatchMouseEvent (or SendInput UIA fallback). Param: x,y.\", \"parameters\": [ { \"name\": \"coords\", \"type\": \"string\", \"required\": true } ], \"examples\": [\"action=RIGHTCLICK path=320,240\"] },");
            sb.AppendLine("    { \"name\": \"DBLCLICK\", \"description\": \"Double left-click at screen coordinates via CDP Input.dispatchMouseEvent (or SendInput UIA fallback). Param: x,y.\", \"parameters\": [ { \"name\": \"coords\", \"type\": \"string\", \"required\": true } ], \"examples\": [\"action=DBLCLICK path=320,240\"] },");
            sb.AppendLine("    { \"name\": \"HOVER\", \"description\": \"Move the cursor to screen coordinates without clicking, via CDP Input.dispatchMouseEvent (or SetCursorPos UIA fallback). Param: x,y.\", \"parameters\": [ { \"name\": \"coords\", \"type\": \"string\", \"required\": true } ], \"examples\": [\"action=HOVER path=320,240\"] },");
            sb.AppendLine("    { \"name\": \"CHECK\", \"description\": \"Check a checkbox by CSS selector (CDP JS el.checked=true + change event). Falls back to UIA TogglePattern when no CDP.\", \"parameters\": [ { \"name\": \"selector\", \"type\": \"string\", \"required\": true } ], \"examples\": [\"action=CHECK path=#agreeTerms\", \"action=CHECK path=input[name=subscribe]\"] },");
